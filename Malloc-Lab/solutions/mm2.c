...
static void mm_check() {
	...
	printf("=====mm_check -- IN ======\n");
	/* print all block */
	for (ptr = free_list_headp; GET_SIZE(HDRP(ptr)) > 0; ptr = NEXT_BLKP(ptr)) {
		printf("Payload addr = %p, Alloc=%d, Size=%d\n", ptr, GET_ALLOC(HDRP(ptr)), GET_SIZE(HDRP(ptr)));
	}
	/* Print free block */
	for (ptr = GET_SUCC(free_list_headp); ptr != free_list_tailp; ptr = GET_SUCC(ptr)) {
		printf("Free block: addr=%p\n", ptr);
	}

	/* Check every block in the free list(Except headp and tailp) marked as free */
	for (ptr = GET_SUCC(free_list_headp); ptr != free_list_tailp; ptr = GET_SUCC(ptr)) {
		if (GET_ALLOC(HDRP(ptr))) {
			printf("This free block(addr=%p) marked as allocated!\n", ptr);
		}
	}
	/* Check if there is any continguous free blocks */
	for (ptr = GET_SUCC(free_list_headp); ptr < GET_PRED(free_list_tailp); ptr = GET_SUCC(ptr)) {
		nextptr = GET_SUCC(ptr);
		if (NEXT_BLKP(ptr) == nextptr) {	/* contingous free block appears */
			printf("Contingous free block appears!====== (%p, %p)\n", ptr, nextptr);
		}
	}
	/* Check every free block actually in the free list */
	for (ptr = GET_SUCC(free_list_headp), r = NEXT_BLKP(free_list_headp); ptr != free_list_tailp; ptr = GET_SUCC(ptr), r = NEXT_BLKP(r)) {
		/* Check next free block */
		while (GET_ALLOC(HDRP(r))) {
			r = NEXT_BLKP(r);
		}
		if (r != ptr) {
			printf("free block: addr=%p is not in free list!\n", r);
		}
	}
...
static void *find_fit(size_t asize)
{
	...
	//printf("find_fit(%u)--OUT(NOT FOUND!)\n", asize);
	return NULL;
}
...
int mm_init(void)
{
	...
	//printf("free_list_headp=%p\n", free_list_headp);
	//printf("free_list_tailp=%p\n", free_list_tailp);
	//mm_check();
	//printf("mm_init--end!\n");
    return 0;
}
...
void *mm_realloc(void *ptr, size_t size)
{
	...
	if (size <= 2*DSIZE) {
		...
		asize = DSIZE * ((size + (DSIZE) + (2*DSIZE) + (DSIZE-1)) / DSIZE);
	}
